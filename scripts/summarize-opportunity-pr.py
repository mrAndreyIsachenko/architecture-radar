#!/usr/bin/env python3
"""Summarize a fresh Opportunity Radar pull request for local review."""
...
REPORT_SUMMARY_SCRIPT = ROOT / "scripts" / "summarize-opportunity-report.py"

spec = importlib.util.spec_from_file_location("summarize_opportunity_report", REPORT_SUMMARY_SCRIPT)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("pr", nargs="?", help="Pull request number or URL. Defaults to newest open opportunity PR.")
...
def is_opportunity_pr(pr: dict[str, object]) -> bool:
    title = str(pr.get("title") or "")
    head = str(pr.get("headRefName") or "")
    return bool(OPPORTUNITY_PR_TITLE_RE.search(title) or OPPORTUNITY_PR_BRANCH_RE.search(head))


def newest_open_opportunity_pr(repo: str) -> str:
    ...
    for pr in data:
        if isinstance(pr, dict) and is_opportunity_pr(pr):
            return str(pr["number"])
    raise SystemExit("no open Opportunity Radar PR found")
...
def changed_files_by_kind(files: list[dict[str, object]]) -> dict[str, list[str]]:
    ...
    for file_info in files:
        ...
        if path.startswith("opportunity-reports/") and path.endswith(".md"):
            kinds["opportunity_reports"].append(path)
        elif path.startswith("opportunities/") and path.endswith(".md"):
            kinds["opportunities"].append(path)
        elif path.startswith("signals/") and path.endswith(".md"):
            kinds["signals"].append(path)
        elif path == "opportunities.json":
            kinds["opportunities_state"].append(path)
        elif path in {"opportunity-interests.md", "opportunity-watchlist.yml"}:
            kinds["opportunity_inputs"].append(path)
        else:
    ...
    for value in parsed_values:
        if value["action"] == "request-fix":
            return {
                ...
                "next_action": (
                    f"request a targeted commercial fix for {value['path']}: "
...
def review_recommendation(summary: dict[str, object]) -> dict[str, str]:
    ...
    if isinstance(changed, dict) and not changed.get("opportunity_reports"):
        return {
            "decision": "needs_manual_review",
            "reason": "no changed opportunity report file was found in the PR",
            "next_action": "confirm whether this is an infrastructure PR rather than an Opportunity Radar report PR",
        }
    if not isinstance(reports, list) or not reports:
        return {
            "decision": "needs_manual_review",
            "reason": "changed opportunity reports could not be summarized",
            "next_action": "inspect the opportunity report files directly",
        }
    if is_draft:
        return {
            "decision": "needs_manual_review",
...
def emit_markdown(summary: dict[str, object]) -> None:
    ...
    if isinstance(changed, dict):
        print("Changed opportunity artifacts:")
        for kind in ("opportunity_reports", "opportunities", "signals", "opportunities_state", "opportunity_inputs", "other"):
    ...
    for report in reports:
        ...
        print(f"Reviewed signals: {report.get('reviewed_signals')} (ledger rows: {report.get('ledger_rows')})")
        print_list("Selected opportunities", report.get("selected_opportunities"))
        print_review_value(report.get("review_value"))
        print_list("Evidence gaps", report.get("evidence_gaps"))
        next_test = str(report.get("recommended_next_test") or "").strip()
        if next_test:
