...

{
  ...
  printf -- '- Include a `## Review Value` report section with exactly one markdown table row covering Verdict, Score, Reason, and Recommended action. Use `merge` only for useful commercial evidence, a useful stage change, or a useful no-candidate/watchlist conclusion; use `request-fix`, `close`, or `watchlist` when the report is stale, weak, duplicate-heavy, or needs targeted repair.\n'
